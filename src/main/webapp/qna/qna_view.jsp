...
<%@ include file = "../common/menu_qna.jsp" %>
	...
	<script>
		...
		function goUpdateAnswer(){
			writeForm.t_gubun.value = "updateAnswer";
    ...
    <main>
        <div class="write-container minW800">
            ...
            <form class="write-form" name="writeForm">
				...
				<c:choose>
					<c:when test="${not empty dto.getA_content() && sessionLevel ne 'top'}">
						<div class="form-group full-width">
		                    <label for="postAnswer">답변</label>
		                    <textarea id="postAnswer" rows="5" readonly>${dto.getA_content()}</textarea>
		                </div>
		                <h4 style = "text-align:right;">
		                	${dto.getA_position()}  |  ${dto.getA_update_date()}
		                </h4>
					</c:when>
	
					<c:when test="${sessionLevel eq 'top'}">
						<div class="form-group full-width">
		                    <label for="postAdmin">답변</label>
		                    <textarea id="postAdmin" name="t_answer" rows="5">${dto.getA_content()}</textarea>
		                </div>
		                <h4 style = "text-align:right;">
                ...
                <div class="form-actions">
                	<c:choose>
                		<c:when test="${sessionLevel eq 'top' && sessionId ne dto.getQ_reg_writer()}">
                			
                		</c:when>
                		<c:when test="${sessionLevel ne 'top' || sessionId eq dto.getQ_reg_writer()}">
                    ...
                    <c:if test="${sessionLevel eq 'top'}">
	                    <button type="button" class="btn-cancel" onclick="goUpdateAnswer()">
	                        <span>답 변 저 장</span>
	                    </button>
                    </c:if>
